...
configureShare({
  showShareImportPopup: _showShareImportPopup,
  renderSidebar,
  startRefreshSpinner: _startRefreshSpinner,
  stopRefreshSpinner: _stopRefreshSpinner,
  buildChannelItem,
  refreshingKeys: _refreshingKeys,
});
...
initSettings({
  renderSidebar,
  buildChannelItem,
  updateFolderPreview: _updateFolderPreview,
  refreshingKeys: _refreshingKeys,
});
...
(function() {
  ...
  // 常時マウス位置を追跡（DOM再構築後の :hover 再評価に使う）
  document.addEventListener('mousemove', function(e) { _lastMX = e.clientX; _lastMY = e.clientY; }, { passive: true });
  ...
  function applyCompact(w, rerender) {
    ...
    if (rerender && wasCompact !== isNowCompact) {
      ...
      renderSidebar();
      // DOM再構築後: Lucide アイコンを再レンダリングして :hover も再評価
      if (!isNowCompact) {
        if (typeof lucide !== 'undefined') lucide.createIcons();
        document.dispatchEvent(new MouseEvent('mousemove', {
          bubbles: true, cancelable: true,
          clientX: _lastMX, clientY: _lastMY,
