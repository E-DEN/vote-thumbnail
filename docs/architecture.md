...
> Agent 向け: `public/js/app.js`（2,876 行）を全読みせず、まずこのファイルで位置を特定し、必要な範囲だけ `read_file` すること。
...
| パス | 行数 | 役割 | 状態 |
| --- | --- | --- | --- |
| `public/js/app.js` | 2,876 | PC ビュー全部 | **分割対象** |
...
| 256–313 | チャンネル選択 | `selectChannel` | `app.js` に残す（コア） |
| 314–882 | ReactionPin PC ビュー | `loadMyPins` `loadReactionSeeds` `postReaction` `renderReactionsHeatmap` `startReactionsLoop` `showMyReactionsPin` `openReactionsMode` `openModalReactions` `renderReactionsPlaylist` `openThumbModal` | **`reactions-view.js`**（判断保留） |
| 883–928 | list / ranking / vote / router / sidebar / share 設定 | `configureListView` `configureRankingView` `configureVoteView` `configureRouter` `configureSidebar` `configureShare` | 各モジュール初期化 |
| 929–1076 | チャンネル追加 | `addChannelFromSidebarInput` | `channel-add.js` |
| 1077–1082 | テーマ設定 | `configureTheme` | `theme.js` 初期化 |
| 1083–1724 | `init()` 本体（約 640 行） | イベント配線の塊 | 各モジュールの `initXxx()` に分配 |
| 1725–1786 | URL デコードペースト / サイドバーイベント / ウェルカム | | `sidebar.js` / `channel-add.js` |
| 1787–1794 | 設定モジュール初期化 | `initSettings` | `settings.js` 初期化 |
| 1795–1864 | タブ / カテゴリフィルタ / チュートリアル | | `router.js` / `vote-view.js` |
| 1865–end | サイドバーリサイズ | | `sidebar.js` |
...
- `init()`（1083–1724）は DOM イベント配線の塊。モジュール抽出時は該当部分を `initXxx()` としてモジュール側に移し、`init()` から呼ぶ。
