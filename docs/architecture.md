...
> Agent 向け: `public/js/app.js`（5,571 行）を全読みせず、まずこのファイルで位置を特定し、必要な範囲だけ `read_file` すること。
...
| パス | 行数 | 役割 | 状態 |
| --- | --- | --- | --- |
| `public/js/app.js` | 5,571 | PC ビュー全部 | **分割対象** |
...
| `public/mobile/js/app.js` | 2,863 | モバイル ビュー（reaction 以外） | 分割候補 |
...
| 3036–3098 | ハッシュルーティング / 画面切替 | `buildHash` `parseHash` `renderCurrentView` `showView` | **`router.js`（Mobile と共通化）** |
| 3099–3359 | 共有リンク・チャンネル追加 | `_postShareLink` `_shareChannelLink` `_shareFolderLink` `_importFromShareCode` `addChannelFromSidebarInput` | **`share.js`** / `channel-add.js` |
| 3360–3373 | テーマ | `applyTheme` | **`theme.js`（Mobile と共通化）** |
| 3374–4036 | `init()` 本体（660 行） | イベント配線の塊 | 各モジュールの `initXxx()` に分配 |
| 4037–4098 | URL デコードペースト / サイドバーイベント / ウェルカム | | `sidebar.js` / `channel-add.js` |
| 4099–4489 | 設定モーダル（390 行） | | **`settings.js`** |
| 4490–4559 | タブ / カテゴリフィルタ / チュートリアル | | `router.js` / `vote-view.js` |
| 4560–end | サイドバーリサイズ | | `sidebar.js` |

## `public/mobile/js/app.js` セクション別マップ

| 行 | セクション | PC 対応物 |
| --- | --- | --- | --- |
| 26–53 | ハッシュ管理 | PC 3036–3060 **ほぼ重複** |
| 54–125 | `selectChannel` `syncChannelMeta` | PC 2404 |
| 126–486 | チャンネルパネル（`_makeChCard` `_makeFolderEl` `renderChannelPanel`） | PC sidebar.js 相当 |
| 487–708 | 共有インポート / 削除ポップアップ / フォルダ名ダイアログ | PC `_importFromShareCode` `_showShareImportPopup` `_showChDelPopup` |
| 709–1342 | D&D | PC `initSidebarDrag` |
| 1343–1535 | チャンネルメニュー・共有・リフレッシュ・削除・追加 | PC 3099–3359 |
| 1536–1602 | タブ切替 / 空状態 | PC `showView` |
| 1603–1915 | 一覧 / 投票 / ランキング | PC 各 view |
| 1916–2018 | 概要シート | PC `openVideoDesc` |
| 2019–2104 | テーマ / 設定モーダル | PC `applyTheme` / 設定モーダル |
| 2105–end | 初期化 | PC `init()` |
...
| 4 | ハッシュ build/parse | 3036–3060 | 27–53 | 引数名 view/tab の違いのみ → `router.js` |
...
| 7 | `applyTheme` | 3363 | 2022 | ほぼ同一 → `theme.js` |
...
- `init()`（3374–4036）は DOM イベント配線の塊。モジュール抽出時は該当部分を `initXxx()` としてモジュール側に移し、`init()` から呼ぶ。
