...
> Agent 向け: `public/js/app.js`（3,890 行）を全読みせず、まずこのファイルで位置を特定し、必要な範囲だけ `read_file` すること。
...
| パス | 行数 | 役割 | 状態 |
| --- | --- | --- | --- |
| `public/js/app.js` | 3,890 | PC ビュー全部 | **分割対象** |
...
| 行 | セクション | 主な関数 | 抽出先案 |
| --- | --- | --- | --- |
| 66–155 | ReactionPin グローバル / 投票適用 | `applyPinPalette` `reactionsComputeKde` `applyVote` `_pollRefresh` `loadRating` | `reactions-view.js` / `vote.js` |
| 156–188 | チャンネル動画ロード / 空状態 | `loadChannelVideos` `_renderEmptyCat` | `channel-view.js` |
| 抽出済み | 投票ビュー | `configureVoteView` `updatePaceGauge` `renderVote` | `vote-view.js`（`_renderEmptyCat` と `applyVote` を注入） |
| 189–256 | メタ・概要欄ヘルパー | `_rebuildRatingRankMap` `openVideoDesc` `closeVideoDesc` `_buildVideoMeta` `_buildPinDot` `_buildReactionsVideoMeta` | `video-meta.js` |
| 257–1243 | サイドバー描画 | `_showShareImportPopup` `_showChDelPopup` `deleteChannel` `deleteFolder*` `_showCompactTooltip` `_showFolderColorPop` `_showCompactRename` `buildChannelItem` `buildFolderItem` `renderSidebar` | **`sidebar.js`**（約 1,000 行） |
| 1244–1301 | チャンネル選択 | `selectChannel` | `app.js` に残す（コア） |
| 1302–1867 | ReactionPin PC ビュー | `loadMyPins` `loadReactionSeeds` `postReaction` `renderReactionsHeatmap` `startReactionsLoop` `showMyReactionsPin` `openReactionsMode` `openModalReactions` `renderReactionsPlaylist` `openThumbModal` | **`reactions-view.js`**（判断保留） |
| 1872–1914 | list / ranking / vote / router / share 設定 | `configureListView` `configureRankingView` `configureVoteView` `configureRouter` `configureShare` | 各モジュール初期化 |
| 1915–2062 | チャンネル追加 | `addChannelFromSidebarInput` | `channel-add.js` |
| 2063–2068 | テーマ設定 | `configureTheme` | `theme.js` 初期化 |
| 2069–2738 | `init()` 本体（約 670 行） | イベント配線の塊 | 各モジュールの `initXxx()` に分配 |
| 2739–2800 | URL デコードペースト / サイドバーイベント / ウェルカム | | `sidebar.js` / `channel-add.js` |
| 2801–2808 | 設定モジュール初期化 | `initSettings` | `settings.js` 初期化 |
| 2809–2878 | タブ / カテゴリフィルタ / チュートリアル | | `router.js` / `vote-view.js` |
| 2879–end | サイドバーリサイズ | | `sidebar.js` |

## `public/mobile/js/app.js` セクション別マップ

| 行 | セクション | PC 対応物 |
| --- | --- | --- | --- |
| 36–107 | `selectChannel` `syncChannelMeta` | PC 1245 |
...
| 9 | `loadMyPins` / `loadReactionSeeds` / `postReaction` | 1313–1354 | reaction.js 46 | API 呼び出しは `reactions-api.js` へ |
...
- **Phase 1（機械的分割・低コストモデル可）**: 上表「抽出先案」の太字モジュールを 1 つずつ切り出す。**1 モジュール = 1 コミット**。関数本体は移動のみ、ロジック変更禁止。`import/export` を足して `eslint` が通ることを確認。
  - 推奨順: ~~`youtube-api.js`~~ → ~~`sidebar-order.js`~~ → ~~`router.js`~~ → ~~`theme.js`~~ → ~~`share.js`~~ → ~~`settings.js`~~ → ~~`list-view.js`~~ → ~~`ranking-view.js`~~ → ~~`vote-view.js`~~ → `reactions-view.js`（判断保留） → ~~`sidebar-drag.js`~~ → `sidebar.js`
...
- `init()`（2069–2738）は DOM イベント配線の塊。モジュール抽出時は該当部分を `initXxx()` としてモジュール側に移し、`init()` から呼ぶ。
